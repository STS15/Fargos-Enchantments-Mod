...
public class CreeperEnchantment extends Item implements ICurioItem {
    ...
    private void spawnStationaryParticles(Player player) {
        if (player.isOnGround()) {
            ...
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(0.0F, 0.5F, 0.0F), 1.0F);
            double radius = 0.5;
            double offsetY = 0.8;
            for (int i = 0; i < 1; i++) {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Grants explosion jump and explode when you get hit").withStyle(ChatFormatting.GRAY));
    }
