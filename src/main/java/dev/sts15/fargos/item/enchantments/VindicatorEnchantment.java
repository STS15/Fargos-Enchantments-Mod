...
public class VindicatorEnchantment extends Item implements ICurioItem {
	
	private final Random random = new Random();

    public VindicatorEnchantment() {
    ...
    private void spawnStationaryParticles(Player player) {
        if (player.isOnGround()) {
            ...
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(0.44F, 0.44F, 0.44F), 1.0F);
            double radius = 0.5;
            double offsetY = 0.8;
            for (int i = 0; i < 1; i++) {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Boosts next attack after switching weapons").withStyle(ChatFormatting.GRAY));
    }
