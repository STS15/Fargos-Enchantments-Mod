...
public class VoidEnchantment extends Item implements ICurioItem {
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        if (!player.level.isClientSide()) {
            spawnStationaryParticles(player);
        }
    }
    
    private void spawnStationaryParticles(Player player) {
        if (player.isOnGround()) {
            ServerLevel serverLevel = (ServerLevel) player.level;
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(0.663F, 0.663F, 0.663F), 1.0F); // Red/orange color for fire
            double radius = 0.5;
            double offsetY = 0.8;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Offers a small chance to avoid lethal damage and negates falling into the void").withStyle(ChatFormatting.GRAY));
    }
