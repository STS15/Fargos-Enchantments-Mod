...
public class WaterEnchantment extends Item implements ICurioItem {
	
	private final Random random = new Random();

    public WaterEnchantment() {
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        if (!player.level.isClientSide()) {
            ...
            spawnStationaryWaterParticles(player);
        }
    }
    ...
    private void spawnStationaryWaterParticles(Player player) {
        if (player.isOnGround()) {
            ServerLevel serverLevel = (ServerLevel) player.level;
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(0.2F, 0.3F, 0.8F), 1.0F); // Red/orange color for fire
            double radius = 0.5;
            double offsetY = 0.8;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Provides infinite air underwater").withStyle(ChatFormatting.GRAY));
    }
