...
public class ThornyEnchantment extends Item implements ICurioItem {
    ...
    private void spawnStationaryParticles(Player player) {
        if (player.isOnGround()) {
            ...
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(0.545F, 0.000F, 0.000F), 1.0F); // Red/orange color for fire
            double radius = 0.5;
            double offsetY = 0.8;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Negate Sweet Berry Bush damage, regen while standing on leaves").withStyle(ChatFormatting.GRAY));
    }
