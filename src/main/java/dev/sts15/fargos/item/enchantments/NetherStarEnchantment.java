...
public class NetherStarEnchantment extends Item implements ICurioItem {
	
	private final Random random = new Random();

    public NetherStarEnchantment() {
    ...
    private void spawnStationaryParticles(Player player) {
        if (player.isOnGround()) {
            ...
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(1.0F, 1.0F, 1.0F), 1.0F); // Red/orange color for fire
            double radius = 0.5;
            double offsetY = 0.8;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Increases crit damage and crit chance").withStyle(ChatFormatting.GRAY));
    }
