...
public class FireEnchantment extends Item implements ICurioItem {
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        if (!player.level.isClientSide()) {
            negateBurningDamage(player);
            spawnStationaryFireParticles(player);
        }
    }

    private void negateBurningDamage(Player player) {
        if (player.isOnFire()) {
            player.clearFire();
        }
    }
    
    private void spawnStationaryFireParticles(Player player) {
        if (player.isOnGround()) {
            ServerLevel serverLevel = (ServerLevel) player.level;
            ParticleOptions particleOptions = new DustParticleOptions(new Vector3f(1.0F, 0.4F, 0.0F), 1.0F); // Red/orange color for fire
            double radius = 0.5;
            double offsetY = 0.8;
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.literal("Passive: Negates all burning damage").withStyle(ChatFormatting.GRAY));
    }
}
